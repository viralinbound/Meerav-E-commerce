...
import { toDisplayProduct } from "../lib/productDisplay";
...
export default function HomeProductGrid() {
  ...
  const filteredProducts = useMemo(() => {
    let result = productsByCategory(selectedCategory).map(toDisplayProduct);
    switch (sortBy) {
      case "price-low":
    ...
    return result;
  }, [selectedCategory, sortBy, productsByCategory]);

  function handleAddToCart(e, display) {
    e.preventDefault();
    e.stopPropagation();
    addItem(display.raw, display.variant, 1);
  }
  ...
  const activeCategory = shopCategories.find((c) => c.id === selectedCategory);
      ...
      <div className="container-max section-padding">
        <div className="text-center mb-10">
          ...
          <p className="text-charcoal-500 max-w-2xl mx-auto">
            {activeCategory ? activeCategory.description : "Every Meerav delicacy in one place — pick your favourites"}
          </p>
        </div>
        ...
        {filteredProducts.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-charcoal-500 mb-4">Try a different category</p>
            <button
              onClick={() => setSelectedCategory(null)}
