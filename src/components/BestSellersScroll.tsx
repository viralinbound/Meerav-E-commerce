...
export function BestSellersScroll({ onProductClick }: BestSellersScrollProps) {
  const { addToCart } = useCart();
  const { products } = useCatalog();
  const scrollerRef = useRef<HTMLDivElement>(null);
  const bestsellers = products.filter((p) => p.isBestseller);
  const rest = products.filter((p) => !p.isBestseller);
  const items = [...bestsellers, ...rest].slice(0, 6);
  ...
  return (
    <section id="best-sellers-scroll" className="py-10 sm:py-14 lg:py-20 bg-white">
